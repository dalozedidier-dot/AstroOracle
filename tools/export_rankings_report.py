...
def build_html(
    ...
    for _, r in df_s.iterrows():
        ...
        rows.append(
            "<tr>"
            f"<td style='padding:10px;vertical-align:top;'><b>{html.escape(cid)}</b><br/>"
            f"<span style='color:#666;font-size:12px;'>RA {html.escape(str(ra))} "
            f"Dec {html.escape(str(dec))}</span><br/>"
            f"<span style='color:#666;font-size:12px;'>pred {html.escape(str(pred))}</span>"
            "</td>"
            f"<td style='padding:10px;vertical-align:top;'>{html.escape(str(rank_v))}</td>"
            f"<td style='padding:10px;vertical-align:top;'>{html.escape(str(u_v))}</td>"
    ...
    th { text-align: left; font-size: 12px; color: #555; border-bottom: 1px solid #eee; padding: 8px 10px; }
...
def main() -> int:
    ...
    ap.add_argument("--candidates", required=True, help="Path to candidates.parquet or candidates.csv")
    ap.add_argument("--cutouts-dir", required=True, help="Directory containing cutout PNGs")
    ap.add_argument("--out", required=True, help="Output file (.html or .pdf)")
    ap.add_argument("--top-k", type=int, default=200)
    ap.add_argument("--embed-images", action="store_true", help="Embed cutouts as base64 in HTML")
    ...
    ap.add_argument("--uncertainty-col", default="", help="Optional: override uncertainty column name")
    ap.add_argument("--pdf-engine", choices=["reportlab"], default="reportlab")
    args = ap.parse_args()

    df = read_candidates(args.candidates)

    id_col = args.id_col or first_existing_col(df, ["id", "source_id", "candidate_id", "obj_id"])
    if not id_col:
        raise SystemExit("No id column found. Provide --id-col.")

    rank_col = args.rank_col or first_existing_col(df, ["rank", "rank_score", "score", "anomaly_score"])
    if not rank_col:
        rank_col = id_col
